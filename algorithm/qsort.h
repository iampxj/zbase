...
#ifndef BASEWORK_QSORT_H_
#define BASEWORK_QSORT_H_

#include "basework/minmax.h"


#define FFSWAP(type,a,b) rte_swap_t(type, a, b)
 ...
 */
#define RTE_QSORT(p, num, type, cmp) do {\
    void *stack[64][2];\
    int sp= 1;\
    stack[0][0] = p;\
    stack[0][1] = (p)+(num)-1;\
 ...
 */
#define RTE_MSORT(p, tmp, num, type, cmp) do {\
    unsigned i, j, step;\
    for(step=1; step<(num); step+=step){\
        for(i=0; i<(num); i+=2*step){\
            unsigned a[2] = {i, i+step};\
            unsigned end = rte_min_t(unsigned, i+2*step, (num));\
