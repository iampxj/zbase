...
OS_THREAD_API int _os_thread_spawn(os_thread_t *thread, const char *name, void *stack,
								   size_t size, int prio, os_thread_entry_t entry,
								   void *arg) {
...
OS_THREAD_API int _os_thread_change_prio(os_thread_t *thread, int newprio, int *oldprio) {
	if (oldprio) {
		*oldprio = 0;
	}
...
OS_SEM_API int _os_sem_timedwait(os_sem_t *sem, int64_t timeout) {
	struct timespec ts;
