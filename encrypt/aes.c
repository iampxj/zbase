...
#include "basework/encrypt/aes.h"
#include "basework/encrypt/aes_internal.h"

#if defined(__GNUC__) || defined(__clang__)
union unaligned_64 { uint64_t l; } __rte_packed;
union unaligned_32 { uint32_t l; } __rte_packed;
union unaligned_16 { uint16_t l; } __rte_packed;
...
const int rte_aes_size= sizeof(AVAES);

struct AVAES *rte_aes_alloc(void)
{
    return general_malloc(sizeof(struct AVAES));
}
...
static inline void addkey(aes_block_t *dst, const aes_block_t *src,
                          const aes_block_t *round_key)
{
    dst->u64[0] = src->u64[0] ^ round_key->u64[0];
    dst->u64[1] = src->u64[1] ^ round_key->u64[1];
}

static inline void addkey_s(aes_block_t *dst, const uint8_t *src,
                            const aes_block_t *round_key)
{
...
static inline void addkey_d(uint8_t *dst, const aes_block_t *src,
                            const aes_block_t *round_key)
{
...
static void subshift(aes_block_t s0[2], int s, const uint8_t *box)
{
...
static inline void mix(aes_block_t state[2], uint32_t multbl[][256], int s1, int s3)
{
...
void rte_aes_crypt(AVAES *a, uint8_t *dst, const uint8_t *src,
...
int rte_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
    ...
    if (!enc_multbl[rte_array_size(enc_multbl) - 1][rte_array_size(enc_multbl[0]) - 1]) {
        j = 1;
        for (i = 0; i < 255; i++) {
    ...
    if (decrypt) {
        ...
        for (i = 0; i < (rounds + 1) >> 1; i++)
            rte_swap_t(aes_block_t, a->round_key[i], a->round_key[rounds - i]);
    }
