 ...
 *   lib_crc16part() logic derived from article Copyright (C) 1986 Stephen
 *    Satchell.
 */
...
uint16_t lib_crc16part(FAR const uint8_t *src, size_t len, uint16_t crc16val)
{
  size_t i;
...
uint16_t __libc lib_crc16(FAR const uint8_t *src, size_t len)
{
  return lib_crc16part(src, len, 0);
}
