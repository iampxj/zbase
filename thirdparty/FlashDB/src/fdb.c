...
fdb_err_t _fdb_init_ex(fdb_db_t db, const char *name, const char *path, fdb_db_type type, void *user_data)
{
    ...
    if (db->file_mode) {
        ...
        if (db->storage.part == NULL) {
            db->storage.part = disk_partition_find(path);
            if (db->storage.part == NULL) {
                FDB_INFO("Error: Partition (%s) not found.\n", path);
                return FDB_PART_NOT_FOUND;
            }
            if (disk_device_open(db->storage.part->parent, &db->storage.f_part.dev)) {
                FDB_INFO("Error: Device (%s) not found.\n", db->storage.part->parent);
                return FDB_PART_NOT_FOUND;
            }
        }
        
        FDB_ASSERT(db->storage.f_part.dev != NULL);
        if (db->sec_size == 0)
            db->sec_size = disk_device_get_block_size(db->storage.f_part.dev);
