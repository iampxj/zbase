 ...
 */
#ifndef BASEWORK_HRTIMER_H_
#define BASEWORK_HRTIMER_H_
...
typedef struct {} ISR_lock_Context;
...
struct hrtimer_context {
	...
	void (*reload)(uint32_t nano_second);
};
...
void _hrtimer_expire(struct hrtimer_context *header, struct hrtimer *first, 
	uint64_t now, ISR_lock_Context *lock_context) {
	do {
		if (first->expire <= now) {
			hrtimer_routine_t routine;
