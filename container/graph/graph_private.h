...
#define graph_err(fmt, ...)   pr_err("%s():%u "fmt, __func__, __LINE__, ##__VA_ARGS__)
#define graph_warn(fmt, ...)  pr_warn("%s():%u "fmt, __func__, __LINE__, ##__VA_ARGS__)
#define graph_info(fmt, ...)  pr_info("%s():%u "fmt, __func__, __LINE__, ##__VA_ARGS__)
#define graph_dbg(fmt, ...)   pr_dbg("%s():%u "fmt, __func__, __LINE__, ##__VA_ARGS__)
...
struct node {
	...
	uint32_t flags;		      /**< Node configuration flag. */
...
#define SZ rte_min(RTE_NODE_NAMESIZE, RTE_GRAPH_NAMESIZE)
	rc = strlcpy(new_name, base_name, SZ);
	if ((size_t)rc == SZ - 1)
		goto fail;
	sz = rc;

    len = rte_max_t(int16_t, (SZ - sz), 0);
	rc = strlcpy(new_name + sz, "-", len);
	if ((size_t)rc == len - 1)
		goto fail;

	sz += rc;
    len = rte_max_t(int16_t, (SZ - sz), 0);
	sz = strlcpy(new_name + sz, append_str, len);
	if ((size_t)rc == len - 1)
		goto fail;
 ...
 *   Pointer to the internal graph object.
 */
void graph_dump(struct printer *pr, struct graph *g);
 ...
 */
void node_dump(struct printer *pr, struct node *n);
