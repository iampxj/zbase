...
__rte_node_register(const struct rte_node_register *reg)
{
	...
	/* Initialize the node */
	if (strlcpy(node->name, reg->name, RTE_NODE_NAMESIZE)  == 
        RTE_NODE_NAMESIZE - 1)
		goto free;
	...
	for (i = 0; i < reg->nb_edges; i++) {
		if (strlcpy(node->next_nodes[i], reg->next_nodes[i],
				RTE_NODE_NAMESIZE) == RTE_NODE_NAMESIZE - 1)
			goto free;
	}
...
node_clone(struct node *node, const char *name)
{
	...
	reg = general_calloc(1, sizeof(*reg) + (sizeof(char *) * node->nb_edges));
	if (reg == NULL) {
		rte_errno = ENOMEM;
		goto fail;
	}
	    ...
	    const char **next_nodes, rte_edge_t nb_edges)
{
	...
	for (i = from; i < max_edges; i++, count++) {
		if (strlcpy(node->next_nodes[i], next_nodes[count],
				RTE_NODE_NAMESIZE) == RTE_NODE_NAMESIZE - 1)
			goto restore;
	}
restore:
...
static void
node_scan_dump(struct printer *pr, rte_node_t id, bool all)
{
	struct node *node;
...
void
rte_node_dump(struct printer *pr, rte_node_t id)
{
	node_scan_dump(pr, id, false);
}

void
rte_node_list_dump(struct printer *pr)
{
	node_scan_dump(pr, 0, true);
}
