...
#define rounddown_pow_of_two(n) (1UL << (fls(n) - 1))
#endif
		...
		size_t esize)
{
	...
	fifo->data = general_calloc(esize, size);
		...
		unsigned int len, unsigned int off)
{
	...
	l = rte_min(len, size - off);

	memcpy((char *)fifo->data + off, src, l);
	memcpy((char *)fifo->data, (const char *)src + l, len - l);
	/*
